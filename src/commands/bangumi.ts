/**
 * pilidown - bangumi command (M9.1)
 * Original: CLI-only, original to pilidown (DownKyi uses WPF window).
 *
 * Usage:
 *   pilidown bangumi <url-or-id> [--season <id>] [--ep <id>] [--quality 127] [--json]
 *
 * Flow:
 *   parseEntrance(<url-or-id>) -> 解析 seasonId / episodeId / mediaId
 *   若提供 --season / --ep，覆盖解析结果
 *   若只有 mediaId（md 号）-> getSeasonByMediaId -> seasonId
 *   若只有 episodeId -> getSeasonInfoByEpisode
 *   getSeasonInfo(seasonId) -> 输出番剧信息 + 剧集列表
 *   若指定 --ep -> getEpisodePlayUrl -> 输出流信息
 */

import type { Command } from 'commander';
import { getSeasonInfo, getSeasonInfoByEpisode, getEpisodePlayUrl, getSeasonByMediaId } from '../api/BangumiApi';
...
import type { BangumiSeasonInfo, BangumiEpisode } from '../types/bili';

interface BangumiCommandOptions {
  season?: string;
  ep?: string;
  quality: string;
  json?: boolean;
...
export function registerBangumiCommand(program: Command): void {
  program
    .command('bangumi <url-or-id>')
    ...
    .action(async (arg: string, opts: BangumiCommandOptions) => {
      ...
      try {
        ...
        if (parsed.type !== 'bangumi') {
          throw new Error(`bangumi command supports bangumi only, got "${parsed.type}"`);
        }

        // 优先级：命令行 --season > parseEntrance seasonId > 通过 mediaId/episodeId 反查
        ...
        if (!seasonId) {
          // 通过 mediaId 反查 seasonId
          if (parsed.mediaId) {
            const media = await getSeasonByMediaId(parsed.mediaId);
            seasonId = media.season_id;
          } else if (epId) {
            // 通过 epId 反查 seasonId
            const season = await getSeasonInfoByEpisode(epId);
            seasonId = season.season_id;
          } else {
            throw new Error('无法确定 season_id：请提供 --season 或包含 season_id/ep_id/md 的 URL');
          }
        }
        ...
        // 输出番剧信息
        if (opts.json) {
          process.stdout.write(JSON.stringify({ event: 'bangumi.info', data: info }) + '\n');
        } else {
        ...
        if (epId) {
          ...
          if (opts.json) {
            process.stdout.write(JSON.stringify({ event: 'bangumi.stream', data: streamResult }) + '\n');
          } else {
            printStreamHuman(streamResult);
          }
        }
      } catch (err) {
        logError('bangumi failed', { error: (err as Error).message });
        process.exitCode = 1;
      }
    });
}

function findEpisode(info: BangumiSeasonInfo, epId: number): BangumiEpisode | undefined {
  // 优先按 id/epid 精确匹配
  const byId = info.episodes.find(e => e.id === epId || e.epid === epId);
  if (byId) return byId;
  // 兜底：尝试 aid 匹配（用户可能传入 AV 号）
  return info.episodes.find(e => e.aid === epId);
}

function printSeasonHuman(info: BangumiSeasonInfo): void {
  console.log(`标题: ${info.title}`);
  console.log(`season_id: ${info.season_id}  media_id: ${info.media_id}`);
  if (info.areas?.length) {
    console.log(`地区: ${info.areas.map(a => a.name).join(', ')}`);
  }
  if (info.up_info) {
    console.log(`UP主: ${info.up_info.uname} (mid=${info.up_info.mid})`);
  }
  console.log(`封面: ${info.cover}`);
  console.log(`统计: 播放 ${info.stat.views}  弹幕 ${info.stat.danmakus}  收藏 ${info.stat.favorites}  投币 ${info.stat.coins}`);
  if (info.evaluate) {
